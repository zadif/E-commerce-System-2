...
public:
	...
	void displayDetails(Table& table , string mode="all") const
	{
		if (!toDelete)
		{
...
public:
	Customer() : customerId(customerIdCounter++), email(""), password(""), address(""), role("customer"), toDelete(false), loyaltyPoints(0), uniqueID(generateUniqueID('c')) {}
...
public:
	...
	void display(string customerID = "null", string sellerID = "null")
	{
		if (customerID != "null")
		{
			...
			OrderItem* temp = front;
			bool found = false;
			table.add_row({ "Order ID", "Product Name", "Quantity", "Status", "Seller ID" , "Total Price" , "Distance" , "Cities to cover"});
			
			while (temp)
			{
				if (temp->customerID == customerID)
				{
					table.add_row({ temp->orderID, temp->product->productName, to_string(temp->quantity), temp->status, temp->sellerID, to_string(temp->quantity * temp->product->productPrice) ,to_string(temp->distanceTravelledByProduct),temp->deliveryCities  });
		...
		else if (sellerID != "null")
		{
			...
			bool found = false;
			Table table;
			table.add_row({ "Order ID", "Product Name", "Quantity", "Status", "Customer ID" , "Total Price" , "Distance" , "Cities to cover" });

			while (temp)
			{
				if (temp->sellerID == sellerID)
				{
					table.add_row({ temp->orderID, temp->product->productName, to_string(temp->quantity), temp->status, temp->customerID, to_string(temp->quantity * temp->product->productPrice) ,to_string(temp->distanceTravelledByProduct),temp->deliveryCities });
...
void updateSeller(int id)
{
	...
	for (auto& seller : sellers)
	{
		if (seller.getSellerId() == id)
		{
			seller.displayDetails(table , "individual");
			int choice;
			string update;
			cout << "1.Edit Email\n2.Edit Password\n3.Edit Address\n4.Go back\n";
			cin >> choice;
